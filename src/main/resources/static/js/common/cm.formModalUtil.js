...
class cm_formModalUtil {
	...
	init(cls, instanceDT, utilForm) {
		var instance = utilForm;
		
		instance.frmCls = cls;
		instance.disable(cls, true);
		instance.setKeyFieldNm($(cls+" .keyfdnm").attr("keyfd"));
		instance.instanceDT = instanceDT;
		
		
		$(cls + " .btn-save").off("click");
		$(cls + " .btn-save").on("click",function () {
			...
			var tmppid = _pid;
			var contentpid = $(instance.instanceDT.tableClass).attr("contentpid");
			if (contentpid != undefined && contentpid != null) {
				tmppid = contentpid;
			}
			
			_curutilDataTable = instance.instanceDT;
			_curMode = "insert";
			var data = instance.getMultipartFormData(instance.frmCls);
			_utilAjax.sendMultiPartAsync("/"+tmppid+"/"+mode, data, success, fail);
			instance.isProcess = false;
		});
...
//		$(cls + " .form-control").attr("disabled",mode);
	...
	formClear(cls) {
		...
		$(cls+" input[type=number].form-control").val("");
