...
function private_function(pos, arg) {
	if (pos=="afterInit") {
		...
		$(".search-panel .form-control[name=comCd]").on("change",function () {
			...
			var data={};
			data.comCd = comCd;
			_utilForm.makeOptionString(".search-panel","menuGid","","선택",_utilAjax.sendSyncRet("/comSysOptMbGroup/list" ,_utilJson.make(_pid, data)));
		})
		$(".form-panel .form-control[name=comCd]").on("change",function () {
			var comCd = $(".form-panel .form-control[name=comCd]").val();
			var data={};
			data.comCd = comCd;
			_utilForm.makeOptionString(".form-panel","menuGid","","선택",_utilAjax.sendSyncRet("/comSysOptMbGroup/list" ,_utilJson.make(_pid, data)));
		})
