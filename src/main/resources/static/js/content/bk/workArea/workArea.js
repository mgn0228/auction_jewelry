...
function private_function(pos, arg) {
	if (pos=="afterInit") {
		...
		var html = " <select class='selectMapAction'><option value=''>동작선택</option><option value='center'>중앙위치</option><option value='point'>경계선</option></select>";
		html += " <input type=\"text\" name=\"addrsearch\"> ";
		html += " <span name=\"btnaddrsearch\" onclick=\"searchAddr();\" class=\"btn btn-primary\">주소이동</span>";
		...
		$("#areaCenterLat").on("change",function () {
			clearMarker();
			drawMarker();
		});
...
//		clearBorderPolygon();
	...
	if (opts.length > 0) {
		...
		if ($("#areaCenterLat").val().trim() == "" || $("#areaCenterLng").val().trim() == "") {
			var moveLatLon = new kakao.maps.LatLng($(opts[0]).attr("lat"),$(opts[0]).attr("lng"));
			map.panTo(moveLatLon);
		} else {
			var moveLatLon = new kakao.maps.LatLng($("#areaCenterLat").val().trim(),$("#areaCenterLng").val().trim());
			map.panTo(moveLatLon);
		}
...
function searchAddr() {
	var searchtxt = $("input[name=addrsearch").val().trim();
